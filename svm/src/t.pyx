...
def performance(y_true, y_pred, metric="accuracy"):
    ...
    y_label[y_label==0] = 1
    positives, negatives = len(filter(lambda x: x == 1, y_label)), len(filter(lambda x: x == -1, y_label))
    assert(positives + negatives == len(y_label))
    if negatives == 0:
        print "negatives is 0, something got fucked up"
    else:
        print "nothing got fucked up"
    ...
    if metric == "accuracy":
        ...
        return cm[1][1]/float(positives)
    else:
        cm = metrics.confusion_matrix(y_true, y_label)
        if negatives == 0:
            print "negatives is 0, something got fucked up"
        return cm[0][0]/float(negatives)
...
def cv_performance(clf, X, y, kf, metric="accuracy"):
    ...
    for train_index, test_index in kf:
        ...
        perf+=performance(y_true=y_test, y_pred=preds, metric = metric)
    # average the predictions
    return perf/float(len(kf))
...
def main() :
    ...
    for metric in metric_list:
        ...
        print "best value of C found: " + str(best_c) + " with metric score: " + str(d[best_c])
        print d
        print "\n"
